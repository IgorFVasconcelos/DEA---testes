...
int wait_for_response_udp(int* verifier)
{
	if (verifier == NULL) {
		delay(1);
		return TRUE;
	...
	if (verifier == &connected) {
		...
		while (!connected && (clock() < start_time + timeout)) {
...
int execute_at_command_udp(char* command, struct sp_port* port, int* verifier)
{
	...
	return wait_for_response_udp(verifier);
}

//Configura o ínicio da rotina mqtt
void configure_udp_routine(struct sp_port* port)
{
restart:
	GOTO_IF_FALSE(
		// Configura internet no chip
		execute_at_command_udp("AT^SXRAT=0\r", port, &ok_received) &&
		execute_at_command_udp("AT+CGDCONT=1,\"IP\",\"java.claro.com.br\"\r", port, &ok_received)
		, restart);
	// Reseta a conexão, se necessário. (Não precisa dar OK)
	execute_at_command_udp("AT^SISC=1\r", port, &ok_received);
	GOTO_IF_FALSE(
		//Inicia a configuração no index 1.
		execute_at_command_udp("AT^SICA=0,1\r", port, &ok_received) &&
		//Define que o protocolo utilizado será o Mqtt
		execute_at_command_udp("AT^SISS=1,srvType,socket\r", port, &ok_received) &&
		//Define o Id da conexão
		execute_at_command_udp("AT^SISS=1,conId,1\r", port, &ok_received)
		, restart);
}
...
void configure_udp_send_routine(struct sp_port* port)
{
	...
	delay(5);
	configure_udp_routine(port);
restart:
	GOTO_IF_FALSE(
		//Define o endereço da conexão
		execute_at_command_udp("AT^SISS=1,address,\"sockudp://" UDP_ENDPOINT "\"\r", port, &ok_received)
		, restart);
}
...
restart:
	GOTO_IF_FALSE(
		//Define o endereço da conexão
		execute_at_command_udp("AT^SISS=1,address,\"sockudp://:6666\"\r", port, &ok_received)
		, restart);
}
...
void start_udp_routine(struct sp_port* port)
{
	while (TRUE)
	{
		...
		(
			//Prepara a conexão
			execute_at_command_udp("AT^SICA=1,1\r", port, &ok_received) &&
			//Efetua a conexão
			execute_at_command_udp("AT^SISO=1\r", port, &ok_received)
			, restart);
		while (TRUE) {
			//Envia a mensagem udp
			execute_at_command_udp("AT^SISW=1,8\r", port, NULL);
			execute_at_command_udp("ABCDEFGH\r", port, &ok_received);
			delay(4);
		}
	restart:
		//Reseta a conexão.
...
restart:
	...
	(
		...
		execute_at_command_udp("AT^SICA=1,1\r", port, &ok_received) &&
		//Efetua a conexão
		execute_at_command_udp("AT^SISO=1\r", port, NULL)
		, restart);
	//AT^SISO=0,1 mostra o ip do listener
	execute_at_command_udp("AT^SISO=1,1\r", port, &ok_received);
	while (TRUE);
}

//Lê a mensagem recebida AT^SISR=1,byteCount
void read_bytes_udp(struct sp_port* port, int byteCount)
{
	char result[20] = "AT^SISR=1,";
	get_command_string(byteCount, result);
	strcat_s(result, 20, "\r");
	execute_at_command_udp(result, port, NULL);
}

void read_serial_internal_udp(struct sp_port* port, int subscribe)
{
	enum sp_return code;
	char buffer[1025];
	...
	do {
		...
		if (code > 0)
		{
			...
			{
				int bytesReceived = verify_if_received_message_udp(buffer + startString);
				if (bytesReceived > 0) {
					read_bytes_udp(port, bytesReceived);
				}
			}
			clear_buffer(buffer, 1025);
		}
...
{
	read_serial_internal_udp(port, 0);
}
...
{
	read_serial_internal_udp(port, 1);
}
